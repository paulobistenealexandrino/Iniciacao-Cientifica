...
# A principal adaptação que eu fiz foi a inserção de um id que permitirá gerar um dataset
# cujas observações são as despesas realizadas, devidamente categorizadas.
...
# Até aqui utilizei o algoritmo original com alterações. O resultado final, daqui em diante
# será diferente. Pois meu objetivo não é gerar uma tabela da despesa média nacional por categoria
# mas ter a lista de despesas para gerar outros níveis de agregação geográfica.
...
# Antes de realizar essa etapa, é necessário executar o script "Gerar Tradutor de Estratos.R"
